...
class Jedi1Parsers extends RegexParsers {
  ...
  private def negate(exp: Expression): Expression = {
    ...
    val zero = Integer(0)
    FunCall(sub, List(zero, exp))//0-<expression>
  }
 ...
 // product ::= term ~ (("*" | "/") ~ term)*
  def product: Parser[Expression] = term ~ rep(("*"|"/") ~ term) ^^ {
     case (t ~ blah) => parseProduct(t, blah)//notice that we are returning blah instead of the second term, this blah is the operand with the operator
  }
 ...
 def integer: Parser[Integer] = """0|(\+|-)?[1-9][0-9]*""".r ^^ {
   case token => Integer(token.toInt)
 }
 // real ::= (\+|-)?[0-9]+\.[0-9]+
 def real: Parser[Real] = """(\+|-)?[0-9]+\.[0-9]+""".r ^^ {
   case token => Real(token.toDouble)
 }
 // boole ::= true | false
