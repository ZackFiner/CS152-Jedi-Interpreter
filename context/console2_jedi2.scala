...
object console2_jedi2 {
   ...
   private def executeFile(fileName: String) {
     ...
     for (line <- Source.fromFile(fileName).getLines if more) {//execute each line
       try {
         if (!line.matches("""\s*""")) {//ignore whitespace
           println("-> " + line)//print the line being executed
           println(execute(line))//print the result
         }
       } catch {
