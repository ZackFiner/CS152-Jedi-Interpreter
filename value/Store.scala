...
case class Store(private var elems: ArrayBuffer[Value] = ArrayBuffer[Value]()) extends Value {
  ...
  // returns "{e0 e1 e2 ...}"
  override def toString = "{"+elems.map(c => c.toString).reduce(_+" "+_)+"}"
  // returns store containing the elements of this transformed by trans
  def map(trans: Closure): Store = Store(elems.map(c => trans.apply(List(c))))
  // returns store containing the elements of this that passed test
  def filter(test: Closure): Store = Store(elems.filter(c => test.apply(List(c))==Boole(true)))
